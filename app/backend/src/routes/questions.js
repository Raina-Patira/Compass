  ...
  try {
    ...
    res.json({
      questions: questions.map((q) => ({
        ...
        title: q.title,
        content:
          q.content.substring(0, 300) + (q.content.length > 300 ? "..." : ""),
        status: q.status,
...
router.get("/:id", authenticateToken, async (req, res) => {
  try {
    const question = await Question.findByPk(req.params.id, {
      include: [
        ...
        },
        {
          ...
          // separate: true, // ✅ ensures proper ordering
          order: [
            ["upvotes", "DESC"],
            ["createdAt", "DESC"],
          ],
    ...
    console.log("YES QUESTUION", question);
    if (!question) {
    ...
    res.json({
      question: {
        ...
        // ✅ FIXED HERE (answers instead of Answers)
        answers:
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // 2️⃣ Extract tags using AI service
      const extractedTags = await aiService.extractTags(title, content);

      console.log("EXTRACTED TAGS:", extractedTags); // debug

      // 3️⃣ Save tags in question_tags
